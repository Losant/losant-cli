...
const utils = {
  ...
  loadConfig: async (file) => {
    ...
    if (appConfig && (!appConfig.appUrl || !appConfig.endpointDomain)) {
      const { endpointDomain, appUrl } = await utils.getWhitelabel({ apiToken: userConfig.apiToken });
  ...
  saveUserConfig: async (config) => {
    ...
    if (has('apiToken', userConfig)) {
      const decoded = jwt.decode(userConfig.apiToken, { complete: true }) || {};
      if (decoded.payload && decoded.payload.iss) {
        const apiKey = decoded.payload.iss !== 'api.getstructure.io' ? decoded.payload.iss : 'https://api.losant.com';
        userConfig[apiKey] = { apiToken: userConfig.apiToken };
      }
      // just delete it, if it cant be decoded.
  ...
  getWhitelabel: async ({ api, apiToken } = {}) => {
    if (!api) {
      api = await getApi({ apiToken });
    }
    let result;
    try {
      result = await api.request({ method: 'get', url: '/whitelabels/domain' });
    } catch (err) {
      if (err.message === 'Invalid access token') {
        utils.logError('Invalid access token please run losant login');
        process.exit(1);
      } else {
        throw err;
      }
