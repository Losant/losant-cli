...
const {
  keyBy, prop, merge, isEmpty, keys, union, pipe, propOr, propEq, find, allPass, gte
} = require('omnibelt');
...
const utils = {
  ...
  hasBootstrapped: pipe(propOr([], 'ftueTracking'), find(allPass([ propEq('name', 'experience'), pipe(propOr(0, 'version'), gte(2)) ])))
