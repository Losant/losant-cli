...
module.exports = async (page, command) => {
  ...
  if (!page) {
    ...
    const pageInfos = await pagesPromise;
    let pageInfo;
    ...
    const result = await api.experienceView.patch({ applicationId, experienceViewId: pageInfo.id, experienceView: { layoutId: keyToId[name] } });
