...
const DEFAULT_URL = process.env.TESTING_URL || 'https://api.losant.com';
...
const utils = {
  ...
  loadConfig: async (file) => {
    ...
    if (appConfig && !appConfig.apiUrl) { appConfig.apiUrl = DEFAULT_URL; }
  ...
  saveUserConfig: async (config) => {
    ...
    if (Object.keys(userConfig).includes('apiToken')) {
      if (key !== DEFAULT_URL) {
        ...
        userConfig[DEFAULT_URL] = { apiToken: token };
      }
  ...
  saveConfig: async (file, config) => {
    if (!config.apiUrl) { config.apiUrl = DEFAULT_URL; }
