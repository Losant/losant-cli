...
const getStatusFunc = ({ apiType, commandType, localStatusParams, remoteStatusParams, getQuery = {} }) => {
  console.log('called status function...');  // eslint-disable-line no-console
  const HEADERS = [
    c.magentaBright('Name'),
    c.magentaBright(commandType === 'experience' ? 'View Type' : 'Directory'),
    c.magentaBright('Local Status'),
    c.magentaBright('Remote Status'),
    c.magentaBright('Conflict')
  ];
  const getStatus = async () => {
    console.log('called get status function...'); // eslint-disable-line no-console
    const { applicationId, apiToken } = await loadConfig();
    if (!applicationId || !apiToken) { return; }
    const api = await getApi({ apiToken });
    console.log('got api...');  // eslint-disable-line no-console
    let items;
    try {
    ...
    console.log('getting remote status...');  // eslint-disable-line no-console
    ...
    console.log('creating columns...');  // eslint-disable-line no-console
    ...
    console.log('about to print....'); // eslint-disable-line no-console
    if (!columns.length) {
