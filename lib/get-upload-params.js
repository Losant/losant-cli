...
const buildMetaDataObj = (item, md5, file) => {
  return {
    id: item.id,
    name: item.name,
    file,
    md5,
    remoteTime: new Date(item.lastUpdated).getTime(),
...
const experienceParams = {
  ...
  postUpsertUpdateMeta: async (view, meta, item) => {
    meta[item.file] = buildMetaDataObj(view, checksum(view.body), item.file);
  }
};
...
const fileParams = {
  ...
  postUpsertUpdateMeta: async (result, meta, item) => {
    ...
    meta[item.file] = buildMetaDataObj(result, s3etag, item.file);
  }
};
