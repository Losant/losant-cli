...
const getUploader = (apiType, commandType, remoteStatusParams, localStatusParams, isConflictDetected, getDeleteQuery, getPatchData, getPostData, postUpsertUpdateMeta) => {
    ...
    try {
      ...
      await Promise.all(localStatus.map(async (stat) => {
        ...
        if (stat.status === 'deleted') {
          if (!command.dryRun) {
            await api[apiType].delete(getDeleteQuery(stat, config));
        ...
        if (!command.dryRun) {
          let action;
          if (stat.id) {
            action = api[apiType].patch(getPatchData(stat, config));
          } else {
            action = api[apiType].post(getPostData(stat, config));
          }
