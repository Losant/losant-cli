...
import { rollbarLog } from './rollbar.js';
...
// Returns undefined for a file that holds no document - empty, whitespace or comments
// only - and throws on malformed yaml. loadAll is used because load() treats a missing
// document as an error rather than an empty result.
const loadYaml = (content) => {
  const documents = [];
  loadAll(content ? content.toString() : '', (doc) => { documents.push(doc); });
  return documents[0];
};
...
const utils = {
  ...
  logProcessing: (message, newline = true) => {
    utils.log(`${c.gray('processing'.padEnd(13))}\t${message}`, newline);
  },
  ...
  logResult: (label, message, color, newline = true) => {
    ...
    let str = `${colorLabel(label.padEnd(13))}`;
  ...
  loadUserConfig: async (checkIsEmpty = true) => {
    ...
    await ensureFile(f);
    let config = loadYaml(await readFile(f)) || {};
    if (config.apiToken) {
  ...
  loadApplicationConfig: async (file, userConfig) => {
    ...
    if (await utils.lockConfig(foundFile)) {
      config = loadYaml(await readFile(foundFile)) || {};
      if (isEmpty(config)) {
        utils.logError('Configuration file missing for this directory, run losant configure to generate this file.');
        return;
      }
      if (!config.apiUrl) {
        ...
        await writeFile(foundFile, dump(config));
      }
  ...
  loadLocalMeta: async (type) => {
    ...
    if (await pathExists(mapFile)) {
      return loadYaml(await readFile(mapFile));
    }
  ...
  saveLocalMeta: async (type, meta) => {
    ...
    const mapFile = path.resolve(dir, `${type}.yml`);
    await ensureDir(dir);
    await writeFile(mapFile, dump(meta));
    return mapFile;
  },
