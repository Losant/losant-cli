...
const {
  capitalize, keys, props, values, compact
} = require('omnibelt');
const { loadConfig, logResult, logError } = require('./utils');
...
const HEADERS_TO_TITLES = {
  version: 'Version Name',
  endpointDefaultCors: 'Default Cors',
  attachedDomains: 'Attached Domains',
  attachedSlugs: 'Attached Slugs',
  creationDate: 'Creation Date',
  lastUpdated: 'Last Updated'
};
const getVersionColumn = props(keys(HEADERS_TO_TITLES));
...
const listVersions = async (api, applicationId, filter) => {
  ...
  const columns = data.items.map((item = {}) => {
    item.attachedDomains = item.attachedDomains ? compact(item.attachedDomains.map((config) => {
      if (config.domainName) {
        return  `http://${config.domainName}`;
      }
    })).join('\n') : '';
    item.attachedSlugs = item.attachedSlugs ? compact(item.attachedSlugs.map((config) => {
      if (config.slug) {
        return `https://${config.slug}.onlosant.com`;
      }
    })).join('\n') : '';
  ...
  printTable(values(HEADERS_TO_TITLES), columns);
};
