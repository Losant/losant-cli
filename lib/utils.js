...
const {
  keyBy, prop, merge, isEmpty, keys, union
} = require('omnibelt');
...
const resolveUserConfig = () => {
  return path.resolve(process.env.HOME || process.env.HOMEPATH || process.env.USERPROFILE, path.join(`${CONFIG_DIR}`, '.credentials.yml'));
};
...
const {
  writeFile, pathExists, readFile, stat, ensureFile, ensureDir
} = require('fs-extra');
...
const utils = {
  ...
  log: (message, newline = true) => {
    console.log('about to log...');  // eslint-disable-line no-console
    ssLog.stdout(message);
    if (newline) { console.log(); } // eslint-disable-line no-console
    console.log('done with log...'); // eslint-disable-line no-console
  },
  ...
  loadUserConfig: async () => {
    ...
    if (isEmpty(config)) {
      ...
      console.log('exiting here...'); // eslint-disable-line no-console
