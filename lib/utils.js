...
const loadYaml = (content) => {
  ...
  return str.trim() ? yaml.load(str) : undefined;
};
...
const utils = {
    ...
    if (config.apiToken) {
      ...
      if (decoded.payload && decoded.payload.iss) {
        ...
        if (wlInfo) {
          ...
          await writeFile(f, yaml.dump(config));
        }
      }
    }
  ...
  loadApplicationConfig: async (file, userConfig) => {
    ...
    if (await utils.lockConfig(foundFile)) {
      ...
      if (!config.apiUrl) {
        ...
        await writeFile(foundFile, yaml.dump(config));
      }
  ...
  saveLocalMeta: async (type, meta) => {
    ...
    const mapFile = path.resolve(dir, `${type}.yml`);
    await ensureDir(dir);
    await writeFile(mapFile, yaml.dump(meta));
    return mapFile;
  },
