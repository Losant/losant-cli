...
module.exports = async (cmd, loadedConfig, applicationInfo) => {
  const { apiToken, apiUrl, applicationId, applicationName, endpointDomain } = loadedConfig ? loadedConfig : await loadConfig();
  const api = await getApi({ apiToken, url: apiUrl });
  try {
