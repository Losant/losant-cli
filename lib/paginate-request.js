...
export default async (request, query, shouldUseOffset = false) => {
  let items = [];
  let extra = {};
  ...
  const results = await request(mergeRight(query, extra)) || { items: [] };
  ...
  if (results.totalCount > 1000) {
    ...
    while (page <= totalPages) {
      extra = (shouldUseOffset && { offset: page*perPage, limit: perPage })|| { page, perPage };
      requests.push(request(mergeRight(query, extra)));
      ++page;
    }
