...
const { merge, values, isEmpty, defer } = require('omnibelt');
...
const getDownloader = ({
  ...
  const download = async (pattern, command = {}, loadedConfig) => {
    ...
    const downloadResults = await allSettledSerial(async (remoteStatus) => {
      ...
      let message = `A conflict has been detected in ${remoteStatus.file}, how do you want to handle this?`;
      if (!command.force) {
        if ( commandType === 'dataTables' ) {
          message = `The file ${remoteStatus.file} already exists locally, how do you want to handle this?`;
          isDataTable = true;
        }
        const localStatus = localStatusByFile[remoteStatus.file];
        const { conflict } = getComparativeStatus(localStatus, remoteStatus, isDataTable);
        if (conflict) {
      ...
      if (!command.dryRun) {
        ...
        let data, contentProperty;
        try {
          data = await getData(itemsById[remoteStatus.id]);
          if (commandType === 'dataTables') {
            contentProperty = data.lastUpdated;
            const deferred = defer();
            const rows = await paginateRequest(api.dataTableRows.get, { applicationId: data.applicationId, dataTableId: data.id });
            CSV.stringify(rows, { header: true }, (err, info) => {
              data = info;
              deferred.resolve();
              if (err) {
                throw err;
