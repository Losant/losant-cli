  ...
  return async (command) => {
    ...
    const timerId = setInterval(async () => {
      if (queue.length && !isRunning) {
        ...
        await forEachSerialP(async () => {
          ...
          const { qPattern, qCommand, qConfig } = queueData.get(file);

          await uploader(qPattern, qCommand, qConfig);
    ...
    watcher.on('change', async (file) => {
      ...
      if (includes(file, queue)) {
        queue.splice(queue.indexOf(file));
      ...
      queue.push(file);
      queueData.set(file, { pattern, command, config });
    });
  };
};
