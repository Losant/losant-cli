...
module.exports = async (page, command) => {
  const { apiToken, applicationId, apiUrl } = await loadConfig();
  ...
  if (!page) {
    ...
    if (pageInfos.length === 1) {
      log(`Only one page found that matched ${page}, ${pageInfos[0].name} ${apiUrl.replace('api', 'app')}/applications/${applicationId}/experience/versions/develop/views/pages/${pageInfos[0].id}`);
    }
    if (pageInfos.length > 1) {
      const pageToInfo = {};
      const choices = pageInfos.map((info) => {
        ...
        const key = ` ${info.name} ${apiUrl.replace('api', 'app')}/applications/${applicationId}/experience/versions/develop/views/pages/${info.id}`;
    ...
    const choices = layouts.map(({ id, name }) => {
      const key = `${name} ${apiUrl.replace('api', 'app')}/applications/${applicationId}/experience/versions/develop/views/layouts/${id}`;
      keyToId[key] = id;
      return key;
    });
