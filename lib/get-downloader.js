...
const getDownloader = ({
  ...
  const download = async (pattern, command = {}, loadedConfig) => {
    ...
    if (pattern) {
      items = items.filter((item) => { return filterFunc(item, command.type); });
    }
    ...
    if (isEmpty(remoteStatusByFile)) {
      return logResult('Not Found', `No ${plural(commandType)} to download from this application`, 'yellow');
    }
    try {
