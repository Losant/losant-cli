...
const experienceParams = {
  ...
  postUpsertUpdateMeta: async (view, meta, localInfo) => {
    meta[localInfo.file] = buildMetaDataObj({ resource: view, md5: checksum(view.body), localInfo });
  }
};
...
const fileParams = {
  ...
  postUpsertUpdateMeta: async (result, meta, localInfo) => {
    const body = await readFile(localInfo.file);
    let s3etag;
    try {
      ...
      throw new Error(`An Error occurred when trying to upload ${localInfo.name} to s3 with the message ${e.message}`);
    }
    meta[localInfo.file] = buildMetaDataObj({ resource: result, md5: s3etag, localInfo });
  }
};
