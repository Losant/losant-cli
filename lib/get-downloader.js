...
const getDownloader = ({
  apiType, commandType, localStatusParams, remoteStatusParams, getData, extraQuery, options = {}
}) => {
  const download = async (pattern, command = {}, loadedConfig) => {
    ...
    const { remoteStatusByFile, localStatusByFile } = options.getStatus ? await options.getStatus(items) : await getStatus({
      commandType,
      items,
      remoteStatusParams,
      localStatusParams,
    ...
    const downloadResults = await allSettledSerial(async (remoteStatus) => {
      ...
      if (!command.force) {
        ...
        const { conflict } = options.getComparativeStatus ? options.getComparativeStatus(localStatus, remoteStatus) : getComparativeStatus(localStatus, remoteStatus);
        if (conflict) {
          const { handleConflict } = await inquirer.prompt([{
            name: 'handleConflict',
            type: 'list',
            message: `A conflict has been detected in ${remoteStatus.file}, how do you want to handle this?`,
            choices: [
          ...
          if (handleConflict === 'local') {
            if (remoteStatus.status !== 'deleted') {
              ...
              meta[remoteStatus.file] = options.getMeta ? options.getMeta(remoteStatus) : {
      ...
      if (!command.dryRun) {
        ...
        let data;
        try {
          data = options.getData ? await options.getData(itemsById[remoteStatus.id], api) : await getData(itemsById[remoteStatus.id], api);
        } catch (e) {
