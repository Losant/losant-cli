...
module.exports = async (page, command) => {
  ...
  const pagesPromise = await paginate(api.experienceViews.get, pageQuery);
  ...
  if (!page) {
    ...
    await allSettledSerial(async (info) => {
      try {
        ...
        logResult('Complete', `${info.name} is now using the layout "${result.layoutName}"`, 'green');
      } catch (e) {
        const message = `Error occured when trying to update page ${info.name} with message ${e.message}`;
        logError(message);
      }
    }, pageInfos);
  }
