...
const utils = {
  ...
  loadUserConfig: async (key) => {
    ...
    if (Object.keys(config).includes('apiToken')) {
      const token = config.apiToken;
      config['https://api.losant.com'] = { apiToken: token };
  ...
  loadConfig: async (file) => {
    ...
    if (appConfig && !appConfig.apiUrl) { appConfig.apiUrl = process.env.TESTING_URL || 'https://api.losant.com'; }
    const apiUrl = appConfig && appConfig.apiUrl;
  ...
  saveUserConfig: async (config) => {
    ...
    await ensureFile(f);
    const userConfig = yaml.safeLoad(await readFile(f)) || {};
    userConfig[key] = config[key];

    if (Object.keys(userConfig).includes('apiToken')) {
      if (key !== 'https://api.losant.com') {
        const token = userConfig.apiToken;
        userConfig['https://api.losant.com'] = { apiToken: token };
      }
  ...
  saveConfig: async (file, config) => {
    if (!config.apiUrl) { config.apiUrl = 'https://api.losant.com'; }
  ...
  getWhitelabel: async (apiInfo) => {
    const api = typeof apiInfo === 'string' ? await getApi({ apiToken: apiInfo }) : apiInfo;
    const req = {
      method: 'get',
      url: '/whitelabels/domain'
    };
