...
const getUploader = ({
  ...
  const uploader = async (pattern, command = {}, loadedConfig) => {
    ...
    const uploadResults = await allSettled(values(localStatusByFile).map(async (stat) => {
      ...
      if (!command.dryRun) {
        ...
        try {
          ...
          const message = `Error occured when attempting to upload to Losant ${stat.file}, with the message ${e.message}`;
          return logError(message);
        }
        try {
