...
describe('Configure Commands', () => {
  it('should prompt the user if there are multiple config urls', async () => {
    ...
    let urls = [];
    const conf = {
      'https://api.losant.space': { apiToken: 'token' },
      'https://api.losant.com': { apiToken: 'token' }
    };
    ...
    nock('https://api.losant.space:443', { encodedQueryParams: true })
      .get('/applications')
      .query({ _actions: 'false', _links: 'true', _embedded: 'true', filterField: 'name', filter: 'help' })
    ...
    stub.onCall(0).callsFake((data) => {
      urls = data[0].choices;
      return Promise.resolve({ url: 'https://api.losant.space' });
    });
    stub.onCall(1).returns(Promise.resolve({ filter: 'help' }));
