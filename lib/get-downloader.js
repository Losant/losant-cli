  ...
  const download = async (pattern, command = {}, loadedConfig) => {
    ...
    const downloadResults = await allSettledSerial(async (remoteStatus) => {
        ...
        if (conflict) {
          ...
          if (handleConflict === 'local') {
            if (remoteStatus.status !== 'deleted') {
              ...
              meta[file] = buildMetaDataObj(remoteStatus, remoteStatus.remoteMd5, { file, localTime: Date.now() });
      ...
      if (!command.dryRun) {
        ...
        meta[file] = buildMetaDataObj(remoteStatus, checksum(data), { file, localTime: Date.now() });
      }
