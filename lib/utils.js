...
const utils = {
  ...
  logProcessing: (message, newline = false) => {
    stdout(`${pad(c.gray('processing'), 13)}\t${message}`);
    writeNewLine(newline);
  },
  ...
  logResult: (label, message, color, newline = true) => {
    ...
    stdout(`${pad(colorLabel(label), 13)}\t${message}`);
    writeNewLine(newline);
  },

  logError: (err, newline = true) => {
    stdout(`${c.red('Error')} ${err.message}`);
    writeNewLine(newline);
  },
