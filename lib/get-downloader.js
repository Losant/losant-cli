  ...
  const download = async (pattern, command = {}, loadedConfig) => {
    ...
    const downloadResults = await allSettledSerial(async (remoteStatus) => {
        ...
        if (conflict) {
          ...
          if (handleConflict === 'local') {
            if (remoteStatus.status !== 'deleted') {
              ...
              meta[remoteStatus.file] = buildMetaDataObj(remoteStatus, remoteStatus.remoteMd5, { file: remoteStatus, localTime: Date.now() });
      ...
      if (!command.dryRun) {
        ...
        meta[remoteStatus.file] = buildMetaDataObj(remoteStatus, checksum(data), { file: remoteStatus.file, localTime: Date.now() });
      }
