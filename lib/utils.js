...
const utils = {
  ...
  loadUserConfig: async (checkIsEmpty = true) => {
    ...
    if (config.apiToken) {
      ...
      if (decoded.payload && decoded.payload.iss) {
        if (decoded.payload.iss === 'https://api.getstructure.io' || decoded.payload.iss === 'http://api.getstructure.io') {
          decoded.payload.iss = 'https://api.losant.com';
        }
        const defaultUrl = decoded.payload.iss;
  ...
  loadApplicationConfig: async (file) => {
    ...
    if (await utils.lockConfig(foundFile)) {
      ...
      if (!config.apiUrl && process.env.LOSANT_API_URL) {
