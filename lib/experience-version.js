...
module.exports = async (version, opts = {}) => {
  ...
  if (!version) {
    ...
    if (isNotNil(opts.slugIds) || isNotNil(opts.domainIds)) {
      const domainAndSlugMap = new Map();
      currentDomains.forEach((d) => {
        domainAndSlugMap.set(d.domainName, d.id);
        domainAndSlugMap.set(d.id, d.id);
      });
      currentSlugs.forEach((d) => {
        domainAndSlugMap.set(d.slug, d.id);
        domainAndSlugMap.set(d.id, d.id);
      });
      let givenSlugs = opts.slugIds?.length ? mapTrim(opts.slugIds.split(',')) : [];
      let givenDomains = opts.domainIds?.length ? mapTrim(opts.domainIds.split(',')) : [];
      versionInfo.slugIds = [];
      let hasErrored = false;
      givenSlugs = givenSlugs.map((slugIdOrName) => {
        if (!domainAndSlugMap.has(slugIdOrName)) {
          hasErrored = true;
          logError(`Slug ${slugIdOrName} was not found in the current list.`);
          return;
        }
        return domainAndSlugMap.get(slugIdOrName);
      });
      givenDomains = givenDomains.map((domainIdOrName) => {
        if (!domainAndSlugMap.has(domainIdOrName)) {
          hasErrored = true;
          logError(`Domain ${domainIdOrName} was not found in te current list.`);
          return;
        }
        return domainAndSlugMap.get(domainIdOrName);
      });
      if (hasErrored) {
        return;
      }
      versionInfo.slugIds = uniq(givenSlugs);
      versionInfo.domainIds = uniq(givenDomains);
    } else {
