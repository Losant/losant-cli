...
const utils = {
  ...
  loadUserConfig: async (checkIsEmpty = true) => {
    ...
    if (config.apiToken) {
      ...
      const defaultUrl = process.env.LOSANT_API_URL;
      // if they do not have api.losant.com api token then map the api token to that otherwise delete it
      if (!config[defaultUrl]) {
        // todo here get whitelabel information
        // which can maybe be hardcoded here :shrug:
        ...
        try {
          ...
          if (isEmpty(config)) {
            utils.logError('Api token is no longer valid, please re-login.');
            process.exit(1);
          }
  ...
  loadApplicationConfig: async (file) => {
    ...
    if (await utils.lockConfig(foundFile)) {
      ...
      if (!config.apiUrl) {
        // old version of the apiUrl
        config.apiUrl = process.env.LOSANT_API_URL;
