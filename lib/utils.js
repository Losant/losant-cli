...
const utils = {
  ...
  getShallowRemoteStatus: async (localFiles, resources, getFilePath) => {
    ...
    resources.forEach((resource) => {
      const file = path.normalize(getFilePath(resource));
      const statObj = {
        id: resource.id,
        file,
        name: resource.name,
  ...
  getRemoteStatus: async (type, resources, getFilePath, contentProperty, options = {}) => {
    ...
    resources.forEach((resource) => {
      const file = path.normalize(getFilePath(resource, metaByFile));
    ...
    metaFiles.forEach((id) => {
      ...
      statusByFile[metaByFile[id].file] = {
        id,
        file,
        name: metaByFile[id].name || path.basename(metaByFile[id].file, type !== 'files' ? path.extname(metaByFile[id].file) : undefined),
