...
const utils = {
  ...
  getShallowLocalStatus: async (type, dir, globPattern) => {
  ...
  getShallowRemoteStatus: async (type, resources, pathTemplate) => {
    const statusByFile = {};
    const meta = await utils.loadLocalMeta(type) || {};
    const metaByFile = {};
    ...
    metaFiles.forEach((id) => {
      ...
      statusByFile[metaByFile[id].file] = {
        id: id,
        file,
        name: path.basename(metaByFile[id].file, path.extname(metaByFile[id].file) || undefined),
        status: 'deleted'
      };
    });
    return statusByFile;
