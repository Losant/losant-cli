...
const {
  keyBy, prop, mergeRight, isEmpty, keys, union, find, propEq, propOr
} = require('omnibelt');
...
const utils = {
  ...
  loadApplicationConfig: async (file, userConfig) => {
    ...
    if (await utils.lockConfig(foundFile)) {
      ...
      if (!config.apiUrl) {
        config.apiUrl = utils.getApiURL(userConfig);
        await writeFile(foundFile, yaml.safeDump(userConfig));
      }
  ...
  getShallowLocalStatus: async (dir, globPattern) => {
    ...
    await Promise.all(files.map(async (file) => {
      ...
      if (!statObj.file) {
        statObj.status = 'missing';
      } else {
        statObj.status = 'found';
      }
  ...
  getShallowComparativeStatus: (localInfo, remoteInfo) => {
    ...
    if ( localStatus === 'found' || remoteStatus === 'missing') {
      conflict = true;
    }
  ...
  getShallowStatus: async ({ items, remoteStatusParams, localStatusParams, dir }) => {

    const localStatusByFile = await utils.getShallowLocalStatus(dir, ...localStatusParams);
    const localFiles = keys(localStatusByFile).map((file) => {
