...
const { writeFile, pathExists, readFile, stat } = require('fs-extra');
const order = (status) => {
...
const utils = {
  ...
  logError: (err, newline = true) => {
    ...
    utils.log(`${c.redBright('Error')} ${err.message ? err.message : err}`, newline);
  },
  ...
  loadConfig: async (file) => {
    ...
    if (!(await pathExists(f))) {
      utils.logError({ message: `Configuration file ${file || 'losant.yml'} does not exist, run losant configure to generate this file.` });
    } else if (await utils.lockConfig(f)) {
      config = yaml.safeLoad(await readFile(f));
      config.file = file;
    }
    return config;
  },
