...
const getUploader = ({
  ...
  const uploader = async (pattern, command = {}, loadedConfig) => {
    ...
    try {
      ...
      if (commandType === 'experience' && (command.type || pattern)) {
        pattern = path.join(path.sep, command.type ? command.type : '**', pattern ? pattern : '*.hbs');
      }
      if (pattern) {
      ...
      if (isEmpty(localStatusByFile)) {
        if (!pattern) {
          return logResult('Missing', `No ${plural(commandType)} found to upload.`);
        } else {
          return logResult('No Matches', `No ${plural(commandType)} found that match this pattern ${pattern}`);
        }
      }
