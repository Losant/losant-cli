...
const { singular } = require('./utils');

const experienceParams = {
  getData: async (view) => {
    return view.body;
  },
  curriedFilterFunc: curry((pattern, view, type) => {
    if (type && view.viewType !== singular(type)) {
      return false;
    }
...
const filesParams = {
  ...
  curriedFilterFunc: curry((pattern, file) => {
    return minimatch(path.normalize(`${file.parentDirectory}${file.name}`), pattern);
  }),
