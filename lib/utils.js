...
const { keyBy, prop, merge, isEmpty } = require('omnibelt');
...
const utils = {
  ...
  loadApplicationConfig: async (file) => {
    ...
    const cwd = parsedPath.dir.split('/');
    cwd.pop();
    process.chdir(cwd.join('/'));
    let config = {};
  ...
  getRemoteStatus: async (type, resources, pathTemplate, contentProperty, options = {}) => {
    ...
    Object.keys(meta).forEach((file) => {
      ...
      metaByFile[item.file] = Object.assign({}, item, { file: file });
      metaFiles.add(item.file);
    });
    ...
    metaFiles.forEach((id) => {
      status.push({
        id: id,
        file: metaByFile[id].file,
        name: path.basename(metaByFile[id].file, path.extname(metaByFile[id].file)),
  ...
  getComparativeStatus: (localInfo, remoteInfo) => {
    ...
    if (localStatus === 'added' && remoteStatus === 'added') {
