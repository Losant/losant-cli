...
export default (nameOfDirectory, interval = 5000) => {
  ...
  return async () => {
    ...
    const close = async function() {
      ...
      // loadConfig above took the lock on the config file, and proper-lockfile keeps
      // refreshing that lockfile until it is released. Dropping the watcher without
      // unlocking leaves the refresh running against a file nobody owns anymore.
      await unlockConfig();
    };
